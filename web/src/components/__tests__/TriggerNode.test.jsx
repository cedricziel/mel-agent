...
describe('TriggerNode', () => {
  ...
  it('should only render output handle (no input)', () => {
    ...
    expect(screen.queryByTestId('handle-target-left-in')).not.toBeInTheDocument()
  })
  ...
  it('should call API when manual trigger button is clicked', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith('/api/agents/test-agent-123/runs/test')
  })
  ...
  it('should handle manual trigger API error', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Failed to trigger manually:', expect.any(Error))
