...
describe('DraftAPI', () => {
  ...
  describe('getDraft', () => {
    it('should fetch draft successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('/api/agents/workflow-123/draft')
    ...
    it('should handle API errors', async () => {
      ...
      await expect(DraftAPI.getDraft('workflow-123')).rejects.toThrow('Failed to get draft: Not Found')
    })
  })
  ...
  describe('updateDraft', () => {
    ...
    it('should handle update errors', async () => {
      ...
      await expect(DraftAPI.updateDraft('workflow-123', { nodes: [], edges: [] }))
  ...
  describe('testDraftNode', () => {
    it('should test node successfully', async () => {
      ...
      const result = await DraftAPI.testDraftNode('workflow-123', 'node-1', { input: 'test' })
  ...
  describe('deployVersion', () => {
    it('should deploy version successfully', async () => {
      ...
      const result = await DraftAPI.deployVersion('workflow-123', 1, 'Deploy notes')
      
      expect(global.fetch).toHaveBeenCalledWith('/api/agents/workflow-123/deploy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          version: 1,
