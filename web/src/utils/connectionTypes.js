...
export const NODE_HANDLE_CONFIG = {
  ...
  default: {
    inputs: [{ id: 'workflow-in', type: HANDLE_TYPES.WORKFLOW_INPUT, position: 'left' }],
    outputs: [{ id: 'workflow-out', type: HANDLE_TYPES.WORKFLOW_OUTPUT, position: 'right' }]
  },
  
  // Agent nodes with configuration inputs
  agent: {
    inputs: [
      { id: 'workflow-in', type: HANDLE_TYPES.WORKFLOW_INPUT, position: 'left' },
      { id: 'model-config', type: HANDLE_TYPES.MODEL_CONFIG, position: 'bottom', offset: '25%' },
      { id: 'tools-config', type: HANDLE_TYPES.TOOLS_CONFIG, position: 'bottom', offset: '50%' },
      { id: 'memory-config', type: HANDLE_TYPES.MEMORY_CONFIG, position: 'bottom', offset: '75%' }
    ],
    outputs: [{ id: 'workflow-out', type: HANDLE_TYPES.WORKFLOW_OUTPUT, position: 'right' }]
  },
  ...
  model: {
    inputs: [],
    outputs: [{ id: 'config-out', type: HANDLE_TYPES.MODEL_CONFIG, position: 'top' }]
  },
  
  tools: {
    inputs: [],
    outputs: [{ id: 'config-out', type: HANDLE_TYPES.TOOLS_CONFIG, position: 'top' }]
  },
  
  memory: {
    inputs: [],
    outputs: [{ id: 'config-out', type: HANDLE_TYPES.MEMORY_CONFIG, position: 'top' }]
  },
  
  // Trigger nodes (output only)
  manual_trigger: {
    inputs: [],
    outputs: [{ id: 'trigger-out', type: HANDLE_TYPES.TRIGGER_OUTPUT, position: 'right' }]
  },
  ...
  if: {
    inputs: [{ id: 'workflow-in', type: HANDLE_TYPES.WORKFLOW_INPUT, position: 'left' }],
    outputs: [
      { id: 'true-out', type: HANDLE_TYPES.WORKFLOW_OUTPUT, position: 'right', offset: '30%' },
      { id: 'false-out', type: HANDLE_TYPES.WORKFLOW_OUTPUT, position: 'right', offset: '70%' }
    ]
  },
...
export function isValidConnection(sourceHandle, targetHandle, sourceNodeType, targetNodeType) {
  const sourceConfig = NODE_HANDLE_CONFIG[sourceNodeType] || NODE_HANDLE_CONFIG.default;
  const targetConfig = NODE_HANDLE_CONFIG[targetNodeType] || NODE_HANDLE_CONFIG.default;
  
  // Find the handle configurations
  const sourceHandleConfig = sourceConfig.outputs?.find(h => h.id === sourceHandle);
  const targetHandleConfig = targetConfig.inputs?.find(h => h.id === targetHandle);
