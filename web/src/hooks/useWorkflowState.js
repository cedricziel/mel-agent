...
export function useWorkflowState(workflowId, enableAutoPersistence = true) {
  ...
  const { scheduleSave, saveNow, isSaving, lastSaved, saveError } =
    ...
    try {
      ...
      if (enableAutoPersistence) {
        try {
          ...
          console.log(
            'No draft found, loading latest version:',
            draftErr.message
          );
        }
      }
      ...
      console.log(
        isDraftMode ? '📝 Draft mode active' : '🚀 Production mode active'
      );
    } catch (err) {
  ...
  const withOptimisticUpdate = useCallback(
    async (optimisticUpdate, apiCall, rollbackUpdate) => {
  ...
  const testDraftNode = useCallback(
    async (nodeId, testData = {}) => {
      if (!isDraft) {
        throw new Error('Node testing is only available in draft mode');
      }

      try {
        const result = await DraftAPI.testDraftNode(
          workflowId,
          nodeId,
          testData
        );
